...
exports.createCart = async (req, res, next) => {
  try {
    ...
    for (i of products) {
      ...
      if (findCartItem) {
        return next(
          new ApiError(409, `this product already ... Id:${productId}`),
        );
      }
...
exports.IncrementAndDecrement = async (req, res, next) => {
  try {
    ...
    if (value === 'increment') {
      ...
      const result1 = await addCart.findOneAndUpdate(
        { userId: req.userid, 'products._id': req.body.id },
        { $set: { 'products.$.quantity': quantity } },
        { new: true },
      );
      return responseHandler(
        200,
        'update cartItems successfully',
        res,
        result1,
      );
    }
...
exports.showCart = async (req, res, next) => {
  try {
    ...
    return responseHandler(
      200,
      `data found / total price for ${result.length} items : ${price}`,
      res,
      result,
    );
  } catch (error) {
